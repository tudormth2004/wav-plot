...
def calc_rms(y, fs, bitdepth, framesize = DEFAULT_FRAMESIZE):
    ...
    print(dbfs)
    dbfs1 = (np.log10(0.2*np.power(fsratio,10))).tolist()
    return t_rms , dbfs , dbfs1
...
def analyze_wav(fn):
    ...
    dbfs1 , dbfs, t_rms = calc_rms(y, fs, bitdepth)
    return dbfs, t_rms, fs , dbfs1
...
def upload_file():
    ...
    if check_ext(file.filename):
        dbfs1 , dbfs, t_rms, fs = analyze_wav(file)
        return jsonify(rms=dbfs , t_rms=t_rms, fs=fs, framesize=DEFAULT_FRAMESIZE)
    elif check_ext(file.filename):
        dbfs1 , dbfs, t_rms, fs = analyze_wav(file)
        return jsonify(rms=dbfs1 , t_rms=t_rms, fs=fs, framesize=DEFAULT_FRAMESIZE)    
    else:
